...
public class Chapter08 {
    ...
    public void testRefillTimeline(Jedis conn) throws InterruptedException {
        ...
        long statusId = Long.parseLong(messages.get(messages.size() - 1).get("id"));
    ...
    public void syndicateStatus(Jedis conn, long uid, long postId, long postTime, double start) {
        ...
        if (followers.size() >= POSTS_PER_PASS) {
            try {
                ...
                executeLater(method, uid, postId, postTime, start);
            } catch (Exception e) {
    ...
    public void refillTimeline(Jedis conn, String incoming, String timeline, double start) {
        ...
        List<Tuple> messages = new ArrayList<>();
        ...
        trans.zremrangeByRank(timeline, 0, -HOME_TIMELINE_SIZE - 1);
        trans.exec();
        ...
        if (users.size() >= REFILL_USERS_STEP) {
            try {
                Method method = getClass().getDeclaredMethod("refillTimeline", Jedis.class, String.class, String.class,
                    Double.TYPE);
                executeLater(method, incoming, timeline, start);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        }
    ...
    public void cleanTimelines(Jedis conn, long uid, long statusId, double start, boolean onLists) {
        ...
        if (followers.size() >= POSTS_PER_PASS) {
            executeLater(method, uid, statusId, start, onLists);
        } else if (!onLists) {
            executeLater(method, uid, statusId, 0, true);
        }
    }

    public void executeLater(Method method, Object... args) {
