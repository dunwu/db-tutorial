...
> :bulb: 更多内容请参：[Redis 持久化](redis-persistence.md)
...
使用 Redis 复制特性，要先保证 RDB 配置选项正确。

- 使用 `slaveof host port` 命令可以让服务器**开始复制**一个新的主服务器。
- 使用 `slaveof no one` 命令可以让服务器**终止复制**操作，不再接受主服务器的数据更新。

一个从服务器只能有一个主服务器，并且不支持主主复制。

#### 复制过程

![](https://raw.githubusercontent.com/dunwu/images/master/snap/20200130101853.png)
