...
- **如果所有数据都是正数，就用 `UNSIGNED`** - `UNSIGNED` 表示不允许负值，大致可以使正数的上限提高一倍。例如：年龄、身高、体重等数据值，明显不会出现负数。
...
- **表的列不宜太多**。原因是存储引擎工作时需要在服务器层和存储引擎层之间通过行缓冲格式拷贝数据，然后在服务器层将缓冲内容解码成各个列，这个转换过程的代价是非常高的。如果列太多而实际使用的列又很少的话，有可能会导致 CPU 占用过高。
